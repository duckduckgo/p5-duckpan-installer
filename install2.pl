...
if ($ENV{PERL_LOCAL_LIB_ROOT} || $ENV{PERL_MM_OPT} || $ENV{PERL_MB_OPT}) {
    ...
    print_text(
    	...
    	"We don't use local::lib anymore, and require you to remove the local::lib calls from your shell startup file, so that we can replace it with perlbrew.",
    );
    ...
    if ($ENV{SHELL} eq '/bin/bash') {
		...
		if (@found) {
			...
			print_text(
				"Found entry for local::lib in $bash_conf_file",
				"Edit this file and remove the line",
				"",
				$extraline,
				"",
				"from it, then restart this script!",
			);
			...
			print_text(
				"You use bash, but we weren't able to find the local::lib startup line in your $bash_conf_file",
			);
		}
	...
	print_text(
		"We consider you know what you do, and so please restart this script, after you removed local::lib from your startup file.",
	);
